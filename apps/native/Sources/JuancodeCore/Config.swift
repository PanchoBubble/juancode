...
public extension Config {
    /// The desktop-side MIRROR of the daemon's session rows, under `dataDir` so a
    /// relocated install keeps everything under one root.
    ///
    /// Not the core's own store: `juancoded` keeps that at
    /// `$JUANCODED_DATA_DIR/juancoded-rust.db` (default `~/.juancode/rust-core`) and
    /// is its only writer. This file is what the sidebar, the search index and the
    /// retention cap read, and the desktop is its only writer — one writer per file,
    /// which is the rule that survives from when there were two cores.
    ///
    /// The name keeps the `-rust` suffix it was given when it was one of two. The
    /// other file, `juancode.db`, is the deleted Swift core's store: nothing reads or
    /// writes it any more, and `juancoded import-swift` is how its history reaches
    /// the daemon (juancode-hebs).
    static var mirrorDatabasePath: String {
        (dataDir as NSString).appendingPathComponent("juancode-rust.db")
    }

    /// Base URL of the `juancoded` daemon (`JUANCODE_RUST_CORE_URL`, else
    /// `JUANCODED_PORT` on loopback, else the daemon's own default port 4290).
    /// Deliberately not 4280: that port belongs to the relay this app serves, and
