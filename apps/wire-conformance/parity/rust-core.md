...
- Status: NO
- Needs: pty
- Why: step 8 (the assertion: the exit frame means the group is gone, not just the child that answered the pty): helper 46012 survived the session by 8000ms. Killing a session has to reap the whole process group: this helper ignores HUP and TERM, so only an escalation to killpg(SIGKILL) reaches it, and not
